...
const int SIZE = 50;
//strlen is not supported with the given headers so I have to create my personal string length checker
//checks for length of string: inputs are pointer to the word character, outputs the number of letters in the word
int len(char *word);
...
int main(int argc, char *argv[])
{
  ...
  while (!infile.eof())
  {
    ...
    //first we search for similarities in characters in the two arrays to obtain substrings eg, bright a substring of brighter
    //initialize i for clarity
    //initialize j

    //return to the user the substring and the full word
    for (int i = 0; i < total_words; i++)
    {
      for (int j = 0; j < total_words; j++)
      {
        ...
        {
          cout << words[j] << " is a substring of " << words[i];
          cout << endl;
        }
      }
    }
...
bool is_equal(char *word1, char *word2)
{
  ...
  //for loop, check if both words arent empty then compare if they are equal
...
bool is_prefix(char *word1, char *word2)
{
  ...
  //compares words and returns false if second word is greater than first word in length
...
char *find_substring(char *haystack, char *needle)
{
  ...
  //initialize two random variables to compare words
  int alphabet_1 = 0;
  int alphabet_2 = 0;
  //implementing written Q4 using is_equal and is_prefix to find substrings
  if (is_equal(haystack, needle))
  {
    return 0;
  }
  alphabet_2 = len(haystack);

  int i = 0;
  for (alphabet_1 = 0; alphabet_1 < alphabet_2; alphabet_1++)
  {
    //if haystack is a prefix of needle, dereference and return first value of haystack
...
{
  int i = 0;
  //if word is not empty add counter i
  while (word[i] != '\0')
  {
    //increment i
    i++;
